...
def process_wsi_to_titan(wsi_path, h5_path, output_path, 
    ...
    # Try hardcoded module name first (current TITAN version)
    titan_module = None
    ...
    # Create transform which accepts numpy arrays directly 
    # (get rid of this since we transform patch at a time, dont need to work with numpy input, just use eval_transform)
    if use_optimized_transform:
        print("Using optimized numpy->tensor transform")
        # Extract normalization constants from eval_transform
...
def main():
    ...
    parser.add_argument('--h5_path', required=True, help='Path to H5 coordinates file')
    parser.add_argument('--output_path', required=True, help='Output path for TITAN embedding (.pt)')
    ...
    if args.skip_existing and os.path.exists(args.output_path):
        print(f"Output file already exists, skipping: {args.output_path}")
        return 0
